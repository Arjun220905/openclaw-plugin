...
# OpenClaw's long-running Gateway reads its global state-dir .env. Read the
# same key for manual script/doctor invocations, without sourcing or executing
# that file and without rejecting unrelated OpenClaw settings it may contain.
if [[ -z "${MEETSTREAM_API_KEY:-}" && -z "${MEETSTREAM_SKIP_DOTENV:-}" ]]; then
  _ms_global_env="${OPENCLAW_STATE_DIR:-${HOME}/.openclaw}/.env"
  if [[ -f "$_ms_global_env" ]]; then
    if [[ -L "$_ms_global_env" ]]; then
      echo "error: refusing to read symlinked credential file: $_ms_global_env" >&2
      exit 78
    fi
    _ms_key_count=0
    _ms_key_value=""
    ...
    if [[ "$_ms_key_count" -gt 1 || ( "$_ms_key_count" -eq 1 && ( -z "$_ms_key_value" || "$_ms_key_value" == *$'\r'* ) ) ]]; then
      echo "error: $_ms_global_env must contain at most one non-empty MEETSTREAM_API_KEY assignment" >&2
      exit 78
    fi
    if [[ "$_ms_key_count" -eq 1 ]]; then
      MEETSTREAM_API_KEY="$_ms_key_value"
      export MEETSTREAM_API_KEY
    fi
  fi
  unset _ms_global_env _ms_key_count _ms_key_value _ms_line
fi
...
ms_request() {
  ...
  if [[ -n "$body" ]]; then
    # Send request bodies over stdin so provider credentials or other sensitive
    # advanced payload fields do not appear in the curl process arguments.
  ...
  if [[ -n "$body" ]]; then
    if ! http_code="$(printf '%s' "$body" | curl "${curl_args[@]}")"; then
      echo "error: request to $url failed (network error)" >&2
      rm -f "$tmp"
      exit 1
    fi
  elif ! http_code="$(curl "${curl_args[@]}")"; then
    echo "error: request to $url failed (network error)" >&2
    rm -f "$tmp"
    exit 1
  fi
