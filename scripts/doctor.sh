...
#
# Checks, in order: required binaries -> API key presence -> live REST auth ->
# live hosted-MCP auth/tool listing -> whether OpenClaw itself can
# see this plugin and skill. Exits 0 when all required checks pass; unavailable optional
...
if [[ -n "${MEETSTREAM_API_KEY:-}" ]]; then
  ...
  record "api_key" "fail" "MEETSTREAM_API_KEY is not set. Add it to ~/.openclaw/.env on the Gateway host, chmod 600 that file, and restart the Gateway."
fi
...
# 4. Hosted MCP auth and tool catalog (read-only)
if [[ -n "${MEETSTREAM_API_KEY:-}" ]] && command -v curl >/dev/null 2>&1 && command -v jq >/dev/null 2>&1; then
  mcp_tmp_dir="$(mktemp -d)"
  chmod 700 "$mcp_tmp_dir"
  mcp_http_code="$(curl -sS -o "$mcp_tmp_dir/response" -w '%{http_code}' --connect-timeout 10 --max-time 60 \
    -H "Authorization: Bearer ${MEETSTREAM_API_KEY}" \
    -H 'Content-Type: application/json' \
    -H 'Accept: application/json, text/event-stream' \
    --data '{"jsonrpc":"2.0","id":1,"method":"tools/list","params":{}}' \
    'https://mcp.meetstream.ai/mcp' 2>"$mcp_tmp_dir/error")"
  mcp_data="$(sed -n 's/^data: //p' "$mcp_tmp_dir/response")"
  if [[ "$mcp_http_code" == "200" ]] && jq -e '.result.tools | length > 0' >/dev/null 2>&1 <<<"$mcp_data"; then
    record "mcp_connectivity" "ok" "Authenticated to https://mcp.meetstream.ai/mcp; $(jq '.result.tools | length' <<<"$mcp_data") tools available"
  elif [[ "$mcp_http_code" == "401" || "$mcp_http_code" == "403" ]]; then
    record "mcp_connectivity" "fail" "Hosted MeetStream MCP rejected the API key (HTTP $mcp_http_code)"
  else
    record "mcp_connectivity" "fail" "Hosted MeetStream MCP probe failed (HTTP ${mcp_http_code:-000})"
  fi
  rm -rf "$mcp_tmp_dir"
else
  record "mcp_connectivity" "skip" "skipped (missing curl, jq, or API key)"
fi

# 5. OpenClaw discovery (best-effort; not fatal if openclaw CLI isn't on PATH)
if [[ "${MEETSTREAM_SKIP_OPENCLAW_DISCOVERY:-}" == "1" ]]; then
  ...
  if openclaw plugins inspect openclaw-x-meetstream-skill --json 2>/dev/null | jq -e '.plugin.id == "openclaw-x-meetstream-skill" and (.mcpServers | length) > 0 and (.diagnostics | length) == 0' >/dev/null \
    && openclaw skills list --json 2>/dev/null | jq -e '.skills[] | select(.name == "meetstream" and .eligible == true)' >/dev/null; then
    record "openclaw_discovery" "ok" "OpenClaw reports the plugin installed and meetstream skill eligible"
  else
    record "openclaw_discovery" "fail" "OpenClaw does not report the plugin and meetstream skill ready. Install the plugin, configure curl/jq and ~/.openclaw/.env, then restart the Gateway."
  fi
else
...
echo "MeetStream OpenClaw plugin doctor"
echo "========================"
for r in "${results[@]}"; do
  IFS='|' read -r check status detail <<<"$r"
  case "$status" in
...
echo
if $overall_ok; then
  echo "All checks passed. Try: Show me the MIA agents I can use."
else
