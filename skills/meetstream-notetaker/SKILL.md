...
```bash
{baseDir}/../../scripts/send-bot.sh --link "<meeting-url>" --name "Meeting Notetaker" --no-video --transcription deepgram --retention-hours 24 --idempotency-key "<uuid>"
```
...
`{baseDir}/../../scripts/get-transcript.sh <bot-id>`. Use the core
`bot-data.sh` for the
...
transcript; consult `{baseDir}/../../references/WEBHOOKS.md`.
