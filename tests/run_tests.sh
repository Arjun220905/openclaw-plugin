...
check_json() {
  local desc="$1" expression="$2" document="$3"
  if jq -e "$expression" >/dev/null 2>&1 <<<"$document"; then
    echo "PASS: $desc"
    pass=$((pass+1))
  else
    echo "FAIL: $desc (jq expression failed: $expression)"
    echo "--- JSON was ---"
    echo "$document"
    echo "----------------"
    fail=$((fail+1))
...
done < <(find "$SKILL_DIR/skills" -name SKILL.md -type f -print)
echo
...
# Isolate from any real local-development .env so this guard test is
...
last_create="$(curl -sS -H "Authorization: Token test-key" "${MEETSTREAM_API_BASE}/test/last-create")"
check_json "deepgram streaming uses nova-2" '.recording_config.transcript.provider.deepgram_streaming.model == "nova-2"' "$last_create"

out="$("$SCRIPTS/send-bot.sh" --link "https://meet.google.com/abc-defg-hij" --name "Assembly Bot" --transcription assemblyai 2>&1)"; code=$?
check "send-bot.sh builds an AssemblyAI bot" 0 "$code"
last_create="$(curl -sS -H "Authorization: Token test-key" "${MEETSTREAM_API_BASE}/test/last-create")"
check_json "AssemblyAI uses universal-2" '.recording_config.transcript.provider.assemblyai.speech_models == ["universal-2"]' "$last_create"

out="$("$SCRIPTS/send-bot.sh" --link "https://meet.google.com/abc-defg-hij" --name "Sarvam Bot" --transcription sarvam 2>&1)"; code=$?
check "send-bot.sh builds a Sarvam bot" 0 "$code"
last_create="$(curl -sS -H "Authorization: Token test-key" "${MEETSTREAM_API_BASE}/test/last-create")"
check_json "Sarvam uses saaras:v3 transcribe" '.recording_config.transcript.provider.sarvam | .model == "saaras:v3" and .mode == "transcribe"' "$last_create"

out="$("$SCRIPTS/send-bot.sh" --link "https://zoom.us/j/123456789" --name "Caption Bot" --transcription meeting_captions 2>&1)"; code=$?
check "send-bot.sh rejects native captions on Zoom" 64 "$code"

out="$("$SCRIPTS/send-bot.sh" --link "https://meet.google.com/abc-defg-hij" --name "Too Short" --max-seconds 599 2>&1)"; code=$?
check "send-bot.sh rejects recordings shorter than 600 seconds" 64 "$code"

out="$("$SCRIPTS/send-bot.sh" --link "https://meet.google.com/abc-defg-hij" --name "Too Long" --max-seconds 18001 2>&1)"; code=$?
check "send-bot.sh rejects recordings longer than 18000 seconds" 64 "$code"
...
echo
echo "== advanced API wrapper tests =="
out="$("$SCRIPTS/api-request.sh" GET /mia 2>&1)"; code=$?
check "api-request.sh performs a documented read" 0 "$code"
check_contains "api-request.sh returns agent data" "Sales Notetaker" "$out"

out="$("$SCRIPTS/api-request.sh" GET '/../secret' 2>&1)"; code=$?
check "api-request.sh rejects path traversal" 64 "$code"

out="$(printf '[]' | "$SCRIPTS/api-request.sh" POST /bots/create_bot --body-file - 2>&1)"; code=$?
check "api-request.sh rejects a non-object JSON body" 64 "$code"

out="$("$SCRIPTS/api-request.sh" DELETE /test/no-content 2>&1)"; code=$?
check "api-request.sh accepts an empty 204 response" 0 "$code"
check_contains "empty 204 response normalizes to JSON" "{}" "$out"
