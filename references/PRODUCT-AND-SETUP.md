...
OpenClaw interprets the user's request locally and invokes the hosted
MeetStream MCP tools or this package's guarded Bash fallbacks; it is not the
hosted bot or MIA runtime. A basic recording bot needs no MIA configuration.
A custom media bridge is a separately hosted service.
...
4. Install the versioned Agent Plugin archive with the command in
   [QUICKSTART.md](../QUICKSTART.md). The archive installs the manifest, hosted
   MCP connection, five skills, scripts, and references as one reviewable unit.
   On Windows, run OpenClaw and the Bash fallbacks in Ubuntu/WSL2. VS Code is
   optional and should use its macOS terminal or a WSL-connected terminal.
5. Put `MEETSTREAM_API_KEY=...` in `~/.openclaw/.env` on the Gateway host,
   protect it with mode `600`, and restart the Gateway. Do not place the key in
   this repository, a command argument, chat, issue, screenshot, or source file.
6. Run `openclaw plugins inspect openclaw-x-meetstream-skill`,
   `openclaw plugins doctor`, and `openclaw skills list`, then start a fresh
   chat. To run the package doctor from a source checkout, use
...
OpenClaw installs the five bundled skills together. If skill allowlists are
configured, include the required skill names. Host environment injection does
not populate a sandbox: provision binaries and the credential in the actual
execution environment used by the agent.
